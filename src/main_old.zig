...
pub fn main() !void {
    ...
    if (builtin.os.tag == .emscripten) {
        // WASM build: use FixedBufferAllocator
        ...
        // Native build: use GeneralPurposeAllocator
...
export fn loop(state: ?*anyopaque) callconv(.c) void {
    const s: *ChessState = @ptrCast(@alignCast(state.?));
    handleInput(s) catch {};
    render(s) catch {};
}

fn handleInput(state: *ChessState) !void {
    // Check for reset key
    ...
    if (mouse_pressed and maybe_square != null) {
        ...
        if (!piece.isEmpty() and piece.getColor() == state.chess.board.active_color) {
            ...
            var all_moves = try state.chess.board.getAllLegalMoves(state.allocator);
